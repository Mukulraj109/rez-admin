  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.tabBar, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        {([
          { key: 'wallet-ops' as TabType, label: 'Wallet Operations', icon: 'wallet-outline' as const },
...
function WalletOpsTab({ colors }: { colors: any }) {
  ...
  const renderUser = useCallback(({ item }: { item: UserWalletItem }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.userCard, { backgroundColor: colors.card, borderColor: isSelected ? colors.tint : colors.border }]}
          ...
          <View style={[styles.modal, styles.auditModal, { backgroundColor: colors.card }]}>
            ...
            {auditLoading ? (
              ...
              <ScrollView style={{ maxHeight: 400 }}>
                ...
                {auditLogs.length === 0 && (
                  <Text style={[styles.emptyText, { color: colors.icon, textAlign: 'center', marginVertical: 24 }]}>No audit entries</Text>
                )}
              </ScrollView>
            )}
